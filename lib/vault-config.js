...
// Built-in template-only fields: present in template frontmatter but must
// NOT appear in document instances (leaked via copy-paste from scaffold).
// Vaults may extend this list via vault-keeper.json `templateOnlyFields`.
const DEFAULT_TEMPLATE_ONLY_FIELDS = [
  'fields',
  'strict',
  'sections',
  'tier',
...
const DEFAULTS = {
  ...
  // D3: extra template-only fields beyond the built-in set
  templateOnlyFields: [],
};
...
export function loadVaultConfig(projectRoot) {
  ...
  // D3: merge user-defined templateOnlyFields with the built-in defaults.
  // User list EXTENDS (not replaces) the built-in set so the defaults are
  // always enforced regardless of vault-keeper.json content.
  const extraTemplateOnlyFields = Array.isArray(fileConfig.templateOnlyFields)
    ? fileConfig.templateOnlyFields.filter((f) => typeof f === 'string')
    : [];
  const mergedTemplateOnlyFields = [...DEFAULT_TEMPLATE_ONLY_FIELDS, ...extraTemplateOnlyFields];
  ...
  const merged = {
    ...
    templateOnlyFields: mergedTemplateOnlyFields,
  };

  Object.freeze(merged.vaultFolders);
  Object.freeze(merged.excludePatterns);
  Object.freeze(merged.templateOnlyFields);
