...
// ── Template rules cache ───────────────────────────────────────────────────
// Keyed by absolute path → { rules, mtime }
// Avoids re-reading + re-parsing the same template file on every doc validation.
// Invalidated per-entry when the file's mtime changes (edit detected on next call).
const _cache = new Map();

/**
 * Clear the in-memory template rules cache.
...
export async function loadTemplateRules(templatePath, projectRoot = process.cwd()) {
  ...
  if (cached && cached.mtime === mtime) {
    return cached.rules; // cache hit — same rules object, no disk read
  }
  ...
  // Store in cache keyed by absPath + mtime for next call
...
export function normalizeRules(rules, source = "inline") {
  ...
  // ── Pre-compile path_regex once (P2) ──────────────────────────────────
  // Stored as _compiledPathRegex so validatePathRegex() skips repeated
  // new RegExp() calls. null = regex string is present but invalid.
  let _compiledPathRegex;
  if (typeof r.path_regex === "string") {
    try {
      _compiledPathRegex = new RegExp(r.path_regex);
    } catch {
      _compiledPathRegex = null; // invalid regex — null sentinel, reported at validation time
    }
  }

  // ── Pre-compile field_rules regexes once (P2) ──────────────────────────
  // Each entry gets _compiledRegex so applyRules() skips repeated new RegExp().
  // null = regex string present but invalid; undefined = no regex on this rule.
  const field_rules = Array.isArray(r.field_rules)
    ? r.field_rules.map((entry) => {
        const rule = { ...entry };
  ...
  return {
    ...
    // Pre-compiled version of path_regex — undefined when no path_regex,
    // null when path_regex is present but invalid.
    ...(_compiledPathRegex !== undefined && { _compiledPathRegex }),
