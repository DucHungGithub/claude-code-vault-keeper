 ...
 *
 * `loadTemplateRules(templatePath)` returns `{ rules, error }`:
 *   - On success: `{ rules: NormalizedRules, error: null }`
 *   - On failure: `{ rules: null, error: "<specific reason>" }`
 *
 * Failure reasons are distinct strings — callers surface them directly so
 * users know whether to fix a missing file, a YAML syntax error, or a
 * missing validation_rules block, instead of guessing from a combined message.
 */
...
export async function loadTemplateRules(templatePath, projectRoot = process.cwd()) {
  if (!templatePath) {
    return { rules: null, error: "Template path is required" };
  }
  ...
  try {
    ...
    return {
      rules: null,
      error: `YAML syntax error in '${templatePath}': ${err.message}`,
    };
  }

  if (!frontmatter || !frontmatter.validation_rules) {
    return {
      rules: null,
      error: `Template '${templatePath}' has no 'validation_rules' block`,
    };
  }
