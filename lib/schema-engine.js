 ...
 * @returns {BodyIssue[]}
 */
export function applyBodySchema(templateBodySchema, docMarkdownBody, docMeta) {
...
function validateChildren(schemaNodes, docNodes, parentContentNodes, ancestorPath, issues) {
  ...
  for (const schema of nonRepeatableSchemas) {
    ...
    if (matchIdx === -1) {
      // Missing section
      if (schema.sectionRules?.required) {
        const severity = schema.sectionRules.severity || "error";
        issues.push({
          level: severity,
          field: path,
          message: schema.sectionRules.message || `Required section '${label}' is missing`,
    ...
    validateSection(schema, docNode, ancestorPath, issues);
  }
  ...
  for (const schema of repeatableSchemas) {
    ...
    for (const docNode of matchingDocNodes) {
      ...
      if (schema.children && schema.children.length > 0) {
        validateChildren(schema.children, docNode.children, docNode.contentNodes, [...ancestorPath, itemLabel], issues);
      }
 ...
 */
function validateSection(schema, docNode, ancestorPath, issues) {
  const label = headingLabel(schema.depth, schema.text);
  const path = headingPath(ancestorPath, label);
  const rules = schema.sectionRules || {};
  ...
  if (schema.children && schema.children.length > 0) {
    validateChildren(schema.children, docNode.children, docNode.contentNodes, [...ancestorPath, label], issues);
  }
}
