...
export const PRIMITIVES = {
  ...
  // F4: state_machine — validate status transitions via previous_status field.
  //
  // Declared on the status field in a template:
  //   status:
  //     type: string
  ...
  //
  // At validation time:
  //   - If `previous_status` is absent in the document → skip (backwards compat).
  //   - If `previous_status` is present but not a key in state_machine → skip
  //     (unknown previous state, likely first commit with explicit previous_status).
  //   - Otherwise check that `status` (value) is in the allowed-next list.
  //
  // This is opt-in: documents without previous_status are never flagged.
  state_machine(value, param, ctx) {
    if (!param || typeof param !== "object" || Array.isArray(param)) return [];
    const level = ctx.severity || "error";
    const field = ctx.field;
    const previousStatus = ctx.frontmatter?.previous_status;

    // No previous_status → skip transition check (backwards compatible).
    if (previousStatus === undefined || previousStatus === null || previousStatus === "") {
      return [];
    }

    const allowedFromPrev = param[String(previousStatus)];
    // Unknown previous state in state machine → skip rather than false-positive.
    if (!Array.isArray(allowedFromPrev)) return [];

    if (!allowedFromPrev.includes(String(value))) {
      const allowed = allowedFromPrev.length > 0 ? allowedFromPrev.join(", ") : "(none — terminal state)";
      return [issue(
        level,
        field,
        ctx.message ||
          `Invalid transition: '${previousStatus}' cannot move to '${value}'. Allowed next: ${allowed}`,
        "invalid-transition",
        `Set '${field}' to one of [${allowed}], or remove 'previous_status' to skip transition check`,
      )];
    }
    return [];
  },
  ...
  for (const [fieldName, fieldSpec] of Object.entries(fields)) {
    ...
    if ("required" in fieldSpec) {
      ...
      if (isRequired) {
        ...
        if (gatePass && (value === undefined || value === null || value === "")) {
          ...
          missingIssue.autoFixable = true;
          missingIssue.fixType = "add-field";
          missingIssue.placeholder = "";
    ...
    for (const [key, rawConstraint] of Object.entries(fieldSpec)) {
      ...
      const ctx = {
        ...
        // F4: full frontmatter for cross-field primitives (e.g. state_machine
        // reads previous_status to validate status transitions)
        frontmatter,
      };
...
export function validateTemplateSchema(fieldsSchema) {
  ...
  for (const [fieldName, fieldSpec] of Object.entries(fieldsSchema)) {
    ...
    for (const [key, rawConstraint] of Object.entries(fieldSpec)) {
      ...
      // Primitives whose constraint IS a plain object by design (not expanded form)
      // must be excluded — otherwise their domain keys (e.g. state names in
      // state_machine) would be mistakenly flagged as unknown modifiers.
      const LITERAL_OBJECT_PRIMITIVES = new Set(["state_machine"]);
      if (!LITERAL_OBJECT_PRIMITIVES.has(key) && rawConstraint !== null && typeof rawConstraint === "object" && !Array.isArray(rawConstraint)) {
      ...
      switch (key) {
        ...
        case "state_machine": {
          // Must be a plain object whose values are all arrays of strings.
          const sm = norm.value;
          if (!sm || typeof sm !== "object" || Array.isArray(sm)) {
            issues.push(issue(
              "error",
              fieldName,
              `'state_machine' on field '${fieldName}' must be an object (state → allowed-next[])`,
              "template-schema-invalid",
              `Example: state_machine:\n  draft: [review]\n  review: [approved, draft]\n  approved: []`,
            ));
          } else {
            for (const [state, nexts] of Object.entries(sm)) {
              if (!Array.isArray(nexts)) {
                issues.push(issue(
                  "error",
                  fieldName,
                  `state_machine['${state}'] on field '${fieldName}' must be an array of allowed-next values`,
                  "template-schema-invalid",
                ));
              } else {
                // Check that each allowed-next is declared as a state
                for (const next of nexts) {
                  if (!(next in sm)) {
                    issues.push(issue(
                      "error",
                      fieldName,
                      `state_machine['${state}'] references undeclared state '${next}' on field '${fieldName}'`,
                      "template-schema-invalid",
                      `Add '${next}' as a key in the state_machine object`,
                    ));
                  }
                }
              }
        ...
        for (const key of Object.keys(rules)) {
          if (!SECTION_RULES_KEYS.has(key)) {
            issues.push(issue(
              "error",
              path,
              `Unknown section-rules key '${key}' in '${path}'`,
              "template-schema-invalid",
